...
function OtherProfile({ userId, onBack }) {
  ...
  const postCount = profile.post_count ?? profile.posts?.length ?? 0;
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        else if (Array.isArray(data.data?.posts)) list = data.data.posts;
    ...
    fetchPosts();
  }, [user?.posts_count]); // Re-fetch posts when user's post count changes
