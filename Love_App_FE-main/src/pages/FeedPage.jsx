...
function CreatePost({ onPost }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onPost(post?.post || post?.data || post);
      ...
      if (refetch) refetch(); // Cập nhật lại thông tin user từ server
      toast("Đã đăng bài thành công! ✨", "success");
    } catch (err) {
      console.error(
        "[feed:create_post_error]",
