...
import { wrapChatMessage } from "../services/emotionChatExtension";
...
function ChatWindow({ match, onBack }) {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const data = await api.get(
        // Thêm /api/ và dấu /
        `/api/chat/${match.id}/history/?limit=50&before_id=${nextCursor}`,
      );
  ...
  const sendMsg = (e) => {
    ...
    wsRef.current.send(JSON.stringify(wrapChatMessage(text)));
    setText("");
  };
