...
async def ensure_post_indexes(db: AsyncIOMotorDatabase):
    """Khởi tạo indexes để tối ưu tốc độ truy vấn."""
    # Index cho user_id giúp count_documents và query profile cực nhanh
    await db.posts.create_index([("user_id", 1)])
    # Index cho created_at giúp load Feed sắp xếp thời gian nhanh hơn
...
async def create_post(
    ...
    # Đảm bảo index luôn tồn tại (idempotent)
    await ensure_post_indexes(db)
    ...
    doc = {
        ...
        "sentiment_score": sentiment, # Giữ lại sentiment_score cho tương thích ngược nếu cần
        "sentiment": sentiment,       # Thêm trường sentiment mới vào database
    ...
    # Sử dụng update_one và kiểm tra matched_count nếu cần debug
