...
async def suggest_sentiment_match(request: Request, limit: int = Query(10, ge=1, le=20)):
    user_id = require_session(request)
    db = get_database()
    try:
        return await suggest_by_sentiment(db, user_id, limit=limit)
...
async def random_match(request: Request, limit: int = Query(10, ge=1, le=20)):
    user_id = require_session(request)
    db = get_database()
    try:
        return await suggest_random(db, user_id, limit=limit)
    except ValueError as e:
        if str(e) == "No available users to match":
            return []
        raise HTTPException(status_code=404, detail=str(e))
